...
KISSY.add('weapons/image-dd/2.0/ddobj', function(S, Base, Template, Anim){
  ...
  ,DATAS = {
    POPUP_HD_TPL: '<div class="box-hd hidden close-rt-wrap" ><a href="#" title="按退出键，也可以关闭哦" class="close-rt J_Close" id="J_CloseImageDD"></a></div>'
  ...
  var o = {
    ...
    ,_showPopupImg: function(srcUrl, srcUrlThunmb){
      ...
      //添加当前图片
      self.popupBd.innerHTML = Template(self.DATA('POPUP_IMG')).render({
      ...
      self.ATTR('activeImg', D.get('IMG', self.popupBd) );//获取新插入的图片的DOM引用
    ...
    ,_initHTMLElement: function(){
      var self = this;
      self.popupOpacityBg = D.get('.img-dd-opacity-bg', self.ATTR('popup'));
      self.popupBox = D.get('.img-dd-box', self.ATTR('popup'));
      self.popupHd = D.get('.box-hd', self.ATTR('popup'));
      self.popupBd = D.get('.box-bd', self.ATTR('popup'));
      self.popupBt = D.get('.box-bt', self.ATTR('popup'));
    }
